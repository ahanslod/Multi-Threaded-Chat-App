...
namespace Sync
{
    ...
    SocketServer::SocketServer(int port)
    {
        ...
        int d = 1;
        if (setsockopt(socketFD, SOL_SOCKET, SO_REUSEADDR, &d, sizeof(int)) < 0)
        {
            throw std::string("Couldn't set socket options\n");
        }
        ...
        if (bind(socketFD, (sockaddr *)&socketDescriptor, sizeof(socketDescriptor)) < 0)
            throw std::string("Unable to bind socket to requested port" + std::to_string(errno));
